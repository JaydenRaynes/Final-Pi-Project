...
Program now contains a fully functional GUI that allows the implementation of functions using buttons within the GUI as opposed to specific controller buttons.
A countdown clock has also been implemented on the 'Record', 'Freeplay', and 'Audio Playback' buttons to allow the user to know how much time they have left within the function.
'''
...
#Creates notes based on frequencies. Can adjust the volume of each note.
#Each note is created with square frequency waves.
...
#Creates a TextString in the top left of the window. Acts a a status bar for the user.
class TextString():
    ...
    #writes the string to the GUI. Only shows up in blue text.
    def write(self):
        screen.blit(self.text, (self.rect.x, self.rect.y))

#Creates a black rectangle in the GUI. Used to cover up previous text so new text can appear clean over it. 
