...
while running:
    ...
    for event in pygame.event.get():                            # Basically reads if you quit
        ...
        for hatName in hat_names:
            ...
            if joystick.get_hat(0) != hatTuple:
                note.stop()
        for octaveName in octave_buttons:
            if (octaveName == "L2"):
                if (joystick.get_button(octaveName) == 1):
                    currentOctave = octaves[currentOctave][-1]
            if (octaveName == "R2"):
                if (joystick.get_button(octaveName) == 1):
                    currentOctave = octaves[currentOctave]
'''
...
# Braden's Task: L2 and R2 being able to change octaves
