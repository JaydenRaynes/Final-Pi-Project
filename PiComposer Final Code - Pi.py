'''
PiComposer code for the Raspberry Pi. PiComposer is an audio recording software that
allows a user to create an audio track, playing notes using a DualShock 4 controller.
A recorded audio track is compiled into a .wav file and can be played back within the
program. These files can also be taken out from the directory for external use if
desired. The program includes a fully interactive GUI, including buttons tied to the
main functions of the program. The GUI also incldues a title card in the bottom-right,
a status bar in the top-left which prompts the user on what to do and a clock that
tells the user how much time is left for recording.
'''
...
#Creates notes based on inputted frequencies. Each note is created with
#square frequency waves.
...
#Creates a button. Differs from a TextString as the draw() function allows
#different collors of text to indicate when a button is selected.
class Button():
...
#Creates a black rectangle in the GUI. Used to cover up previous text
#so new text can appear clean.
class Rect():
...
#Organizes every note into a dictionary of dictionaries for quick and uniform access
#to each note. Each key-value pair contains two values: an index and a note.
#The index is used to tie a specific note to a button on the controller.
...
def create_octaves(octaves, button_names, hat_names, hat_tuples, music_button_names):
    ...
    for octave in octaves.keys():
        ...
        i = 0
        #D-Pad is considered a 'hat,' and therefore mappings require tuples.
        ...
        for index in range(len(hat_names)):
            ...
            total_index += 1
        #Ties the rest of the notes in the octave to the remaining
...
#Initializes the program by clearing all previous
#audio files and initiation the audio stream for recording.
def initialize():
    #clears the directory of all previous audio output files
    print("Clearing Cache...")
    path = "/home/pi/Documents/PiComposer/"
...
def updateTime(CLOCK, CLOCK_REFRESH, startTime):
    TIME_LEFT = Button(600, 5, "Time Left:")
...
#Allows for the recording of the audio played within the function and puts
#it into an array. The array is compiled into a .wav file that can be
#accessed elsewhere in the program.
def recording(output_filename):
    CLOCK = Button(750, 5, RECORD_SECONDS)
    CLOCK_REFRESH = Rect(750, 5, 100, 30)
    startTime = time.time()
    currentOctave = 'octave_3'
    frames = []
    joystick.init()
    while True:
        #reads all outputted audio from the speakers and records it
        #in chunks of 1024 frames.
...
def freeplay():
    ...
    while True:
        ...
        #highlights certain LEDs to indicate which octave is active
        if (currentOctave == "octave_3"):
...
#Plays back the latest audio file that was recorded. If nothing has been recorded
#prior to the implementation of this function, it prints a notice to the user in the GUI.
...
# setup the output for octaves LEDs
...
record_button = Button(25, 400, "Record")
freeplay_button = Button(380, 400, "Freeplay")
playBack_button = Button(150, 400, "Playback Audio")
selectorRecord = 0
selectorPlay = 0
selectorplayBack = 0

bottom_border = Rect(0, 400, 800, 50)
top_border = Rect(0, 0, 800, 50)
...
while running:
    ...
    for event in pygame.event.get():
        ...
        #logic on how to color each button to highlight the current button selected
        #number 0 = white, number 1 = red
        ...
        #highlights corresponding button green.
        if joystick.get_button(0) == 1:
            ...
            #begins freeplay
                ...
                freeplay()
                selectorPlay = 0
            #loads and plays the audio track into the pygame mixer
